/**
 * @file QMeshProjector.cpp
 * @author Ryan P. Daly (rdaly@herzog.com)
 * @brief QMeshProjector class which traces objects
 * @version 0.1
 * @date 2022-08-18
 * 
...
lidarshooter::QMeshProjector::QMeshProjector(ros::Duration __publishPeriod, ros::Duration __tracePeriod, std::shared_ptr<spdlog::logger> __logger, QObject* _parent)
      ...
      QObject(_parent)
{
    ...
    _logger->info("Starting up QMeshProjector");
    ...
    _meshSubscriber = _nodeHandle.subscribe<pcl_msgs::PolygonMesh>("/objtracker/meshstate", MESH_SUB_QUEUE_SIZE, &QMeshProjector::meshCallback, this);
    _joystickSubscriber = _nodeHandle.subscribe<geometry_msgs::Twist>("/joystick/cmd_vel", JOYSTICK_SUB_QUEUE_SIZE, &QMeshProjector::joystickCallback, this);
    _publishTimer = _nodeHandle.createTimer(_publishPeriod, std::bind(&QMeshProjector::publishCloud, this));
    _traceTimer = _nodeHandle.createTimer(_tracePeriod, std::bind(&QMeshProjector::traceMeshWrapper, this));
...
lidarshooter::QMeshProjector::QMeshProjector(const std::string& _configFile, ros::Duration __publishPeriod, ros::Duration __tracePeriod, std::shared_ptr<spdlog::logger> __logger, QObject* _parent)
    : _nodeHandle("~"), _publishPeriod(__publishPeriod), _tracePeriod(__tracePeriod),
      _device(rtcNewDevice(nullptr)), _scene(rtcNewScene(_device)),
...
lidarshooter::QMeshProjector::QMeshProjector(std::shared_ptr<LidarDevice> _configDevice, ros::Duration __publishPeriod, ros::Duration __tracePeriod, std::shared_ptr<spdlog::logger> __logger, QObject* _parent)
    : _nodeHandle("~"), _publishPeriod(__publishPeriod), _tracePeriod(__tracePeriod),
      _device(rtcNewDevice(nullptr)), _scene(rtcNewScene(_device)),
      _meshWasUpdated(false), _meshWasUpdatedPublic(false),
      _stateWasUpdated(false), _stateWasUpdatedPublic(false),
...
lidarshooter::QMeshProjector::~QMeshProjector()
{
...
void lidarshooter::QMeshProjector::shutdown()
{
    // Now sure if we need to shut down the node manually
    _nodeHandle.shutdown();
}

void lidarshooter::QMeshProjector::meshCallback(const pcl_msgs::PolygonMesh::ConstPtr& _mesh)
{
...
void lidarshooter::QMeshProjector::setMesh(const pcl::PolygonMesh::ConstPtr& _mesh)
{
...
sensor_msgs::PointCloud2ConstPtr lidarshooter::QMeshProjector::getCurrentStatePtr() const
{
    return _currentState;
}

void lidarshooter::QMeshProjector::getCurrentStateCopy(pcl::PCLPointCloud2::Ptr& _output)
{
...
void lidarshooter::QMeshProjector::traceMeshWrapper()
{
...
void lidarshooter::QMeshProjector::joystickCallback(const geometry_msgs::Twist::ConstPtr& _vel)
{
...
void lidarshooter::QMeshProjector::publishCloud()
{
...
bool lidarshooter::QMeshProjector::meshWasUpdated()
{
...
bool lidarshooter::QMeshProjector::cloudWasUpdated()
{
...
inline Eigen::Vector3f lidarshooter::QMeshProjector::transformToGlobal(Eigen::Vector3f _displacement)
{
...
void lidarshooter::QMeshProjector::updateGround()
{
...
void lidarshooter::QMeshProjector::updateMeshPolygons(int frameIndex)
{
...
void lidarshooter::QMeshProjector::traceMesh()
{
...
void lidarshooter::QMeshProjector::getMeshIntersect(int *_valid, RayHitType *_rayhit)
{
    GET_MESH_INTERSECT(_valid, _rayhit);
}

void lidarshooter::QMeshProjector::getMeshIntersect1(float ox, float oy, float oz, float dx, float dy, float dz, RTCRayHit *rayhit)
{
...
void lidarshooter::QMeshProjector::getMeshIntersect8(const int *validRays, RTCRayHit8 *rayhit)
{
    RTCIntersectContext context;
    rtcInitIntersectContext(&context);
...
void lidarshooter::QMeshProjector::getMeshIntersect16(const int *validRays, RTCRayHit16 *rayhit)
{
    RTCIntersectContext context;
    rtcInitIntersectContext(&context);
...
void lidarshooter::QMeshProjector::setupObjectGeometryBuffers(int _numVertices, int _numElements)
{
...
void lidarshooter::QMeshProjector::setupGroundGeometryBuffers(int _numVertices, int _numElements)
{
...
void lidarshooter::QMeshProjector::releaseObjectGeometryBuffers()
{
...
void lidarshooter::QMeshProjector::releaseGroundGeometryBuffers()
{
    // Allow embree to do its thing
